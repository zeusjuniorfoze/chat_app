...
if (isset($_POST['login'])) {
    ...
    if ($is_admin > 0) {
        ...
        if ($existing_user_count > 0) {
            $erre = "Cet utilisateur est déjà dans le groupe.";
        } else {
            ...
            $insert_sql = $con->prepare("INSERT INTO user_group (ID_ADMIN_AJOUT, ID, IDG, TYPE_USER_GROUP, DATE_AJOUT)
                                            VALUES (?, ?, ?, ?, ?)");
            $insert_sql->execute(array($id_user, $user, $id_group, "user", date('d/m/y')));
?>
            <script type="text/javascript">
                alert(" Utulisateur ajouter avec succes !");
                window.location = "info_admin_group.php";
            </script>
<?php  }
    } else {
            ...
            <div class="formbox">
                <form id="ins" method="POST" action="">
                    ...
                    <div class="inputbox">
                        ...
                        <a href="info_admin_group.php" class="btn cancel-button">Annuler</a>
                    </div>
